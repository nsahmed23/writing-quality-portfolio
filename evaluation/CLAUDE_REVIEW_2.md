...
1. **D1 CONFIRMED — the recorded Windows command was not self-contained.** The checkpoint's `python -m unittest discover -s tests` omitted the required `PYTHONPATH=src` environment and per-runtime interpreter pinning. Falsification reproduced by this reviewer on Linux in a fresh process with `PYTHONPATH` unset: `Ran 139 tests ... FAILED (errors=9, skipped=9)`, exit 1 — matching the Codex-reported Windows falsification (139 tests, 9 `ModuleNotFoundError: No module named 'wqeval'`). Checkpoint 05 now stores the two exact PowerShell commands with `PYTHONPATH=src` and `py -3.11`/`py -3.14 -X utf8` pinning, labeled **reproduced** rather than original because the original invocation was not preserved, plus a `superseded_command_record` documenting the falsified prior command. The Linux commands were already self-contained (`PYTHONPATH=src` inline) and are labeled observed.
...
3. **D3 CONFIRMED — the PR #3 body carried stale metadata** (a hard-coded revision number and head SHA). Fixed on the PR body by removing the hard-coded labels; PR metadata only, no repository content involved, and the fixed state is observed mutable external state on GitHub, not tree-verifiable.

The verdict is again unchanged.

Revision 5 (2026-08-27), in response to the Codex re-verification of head `71bb88d`. One finding, confirmed after the required occurrence inventory:

1. **C1 CONFIRMED: revision 4's D2 completeness claim was wrong.** The full inventory of current assertions that the PR #2 description was corrected contains three occurrences, not the two revision 4 item 2 claimed: the R3 finding text (previously "since corrected", unqualified), the section 4 preamble, and revision 2 item 1 (both already qualified). The R3 occurrence now reads "whose correction is currently observed mutable external state on GitHub and is not tree-verifiable". Revision 4 item 2 stands as superseded historical wording; this entry corrects its inventory.
2. Both nonblocking precision notes applied: revision 4 item 1 now attributes the Windows falsification as Codex-reported, and revision 4 item 3 labels the fixed PR #3 body as observed mutable external state. This revision entry avoids new em dashes per the workspace writing convention; em dashes already present in earlier revisions are preserved as historical text.
